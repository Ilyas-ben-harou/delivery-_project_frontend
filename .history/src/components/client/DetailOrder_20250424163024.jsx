  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex items-center mb-6">
        ...
        <span className={`ml-4 px-3 py-1 text-sm font-semibold rounded-full ${getStatusClasses(order.status)}`}>
          {order.status}
        </span>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="font-medium">{formatDate(order.collection_date)}</div>
            ...
            <div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="font-medium">{order.product_width} × {order.product_height}</div>
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="font-medium">{order.customer_info.address || 'N/A'}</div>
                
                <div className="text-gray-600">City:</div>
                <div className="font-medium">{order.customer_info.city || 'N/A'}</div>
                
                <div className="text-gray-600">Postal Code:</div>
                <div className="font-medium">{order.customer_info.postal_code || 'N/A'}</div>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="text-gray-600">Phone:</div>
                <div className="font-medium">{order.livreur.phone || 'N/A'}</div>
                
                <div className="text-gray-600">Vehicle:</div>
                <div className="font-medium">{order.livreur.vehicle_type || 'N/A'}</div>
              </div>
            </div>
          </div>
        )}
                ...
                {order.history.map((event, index) => (
                  <div key={index} className="relative pl-10">
                    ...
                    <div className="flex flex-col">
                      ...
                      {event.user && (
                        <span className="text-xs text-gray-500">By: {event.user.name}</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        {order.deliveryDocument && (
          <div className="p-6">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Delivery Document</h2>
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-gray-900">{order.deliveryDocument.title || 'Delivery Document'}</p>
                  <p className="text-sm text-gray-500">
                    {order.deliveryDocument.created_at ? formatDate(order.deliveryDocument.created_at) : 'N/A'}
                  </p>
                </div>
